...
for (const element of allh2elements){
    element.style.color = 'lightblue'
}

// ৪. card ক্লাস যতগুলা জায়গায় ইউজ করা হয়েছে সবগুলা কার্ড এর বর্ডার রেডিয়াস ৩০px করে দিতে পারো কিনা 

const cardB = document.getElementsByClassName('card');
for (element of cardB){
...
const buttonItem = document.getElementsByClassName('btn');
for (const itemButton of buttonItem){
    itemButton.addEventListener('click',function(dltBtn){
        dltBtn.target.parentNode.removeChild(dltBtn.target);
    })
};
...
document.getElementById('exampleInputEmail1').addEventListener('keyup',function(event){
    ...
    const enableButton = document.getElementById('button-onOff');
    if (text == 'email'){
...
document.getElementById('chngImg').addEventListener('mouseenter',function(){
    document.getElementById('mouseUse').src = "images/shoes/shoe-1.png";
})
